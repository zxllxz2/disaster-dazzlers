CREATE TABLE "incidents-database"."incidents_nearest_weather" WITH (format = 'parquet') AS WITH weather_aggregated AS (
  SELECT MIN(w.station_id) AS station_id,
    w.timestamp_utc,
    AVG(w.temp) AS avg_temperature,
    AVG(w.rh) AS avg_humidity,
    AVG(w.wind_spd) AS avg_wind_speed,
    AVG(w.slp) AS avg_sea_level_pressure,
    AVG(w.app_temp) AS avg_apparent_temperature,
    AVG(w.solar_rad) AS avg_solar_radiation,
    AVG(w.pres) AS avg_pressure,
    AVG(w.dewpt) AS avg_dewpoint,
    AVG(w.snow) AS avg_snow,
    AVG(w.uv) AS avg_uv_index,
    AVG(w.precip) AS avg_precipitation,
    AVG(w.clouds) AS avg_cloud_coverage,
